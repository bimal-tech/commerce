...
def watchlist(request):
    ...
    i = object_calculations(request.user.username)
    present = False
    prodlst = []
    if lst:
        present = True
...
def createlisting(request):
    ...
    if request.method == "POST":
        ...
        if request.POST.get('image_link'):
            ...
            item.image_link = "https://st4.depositphotos.com/14953852/24787/v/600/depositphotos_247872612-stock-illustration-no-image-available-icon-vector.jpg"
...
def closebid(request, product_id):
    ...
    if not obj:
        ...
        winobj.image_link = listobj.image_link
        winobj.save()
        message = "Bid Closed"
        msg_type = "success"
        # removing from Bid
...
def object_calculations(username):
    lst = Watchlist.objects.filter(user=username)
    i = 0
